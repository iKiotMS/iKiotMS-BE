...
class WarehouseQueryDTO {
  ...
  validate() {
    ...
    if (this.status && !Object.values(WAREHOUSE_STATUS).includes(this.status)) {
      errors.push(`Invalid status filter. Must be one of: ${Object.values(WAREHOUSE_STATUS).join(", ")}`);
    }
...
module.exports = WarehouseQueryDTO;
